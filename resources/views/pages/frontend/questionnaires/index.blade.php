        ...
        <form id="questionnaireForm" action="{{ route('landing.questionnaire.store') }}" method="POST">
            ...
            @foreach ($questionnaires as $index => $questionnaire)
                <div class="question {{ $loop->first ? 'active' : '' }}" data-question="{{ $index + 1 }}">
                    ...
                    @foreach ($questionnaire->learningCategoryQuestionnairies as $learningCategoryQuestionnaire)
                        <button type="button" class="btn answer-btn" data-questionnaire-id="{{ $questionnaire->id }}" data-category-questionnaire-id="{{ $learningCategoryQuestionnaire->id }}">
                            {{ $learningCategoryQuestionnaire->description }}
                        </button>
                    @endforeach
